...
Next I examine new AR models to quantify the relationship between the independent variables and better understand their relative effects on chlorophyll concentration. Given that in the previous AR model only the first lagged chlorophyll variable was of strong practical significant, I've decided to only use one lag for this model to maximize the chances that we observe a relationship that is practically significant. Each independent variable is lagged one period.
